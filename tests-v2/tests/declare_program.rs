...
fn setup() -> (LiteSVM, Keypair) {
    ...
    build_program(
        test_dir
            .join("programs/declare-program/programs/external-cpi")
            .to_str()
            .unwrap(),
        deploy_str,
    );
    build_program(
        test_dir
            .join("programs/declare-program/programs/alt-cpi")
            .to_str()
            .unwrap(),
        deploy_str,
    );
    build_program(
        test_dir
            .join("programs/declare-program/programs/hash-cpi")
            .to_str()
            .unwrap(),
        deploy_str,
    );
    build_program(
        test_dir.join("programs/declare-program").to_str().unwrap(),
        deploy_str,
    );
    ...
    svm.add_program_from_file(caller_id(), &deploy_dir.join("declare_program.so"))
