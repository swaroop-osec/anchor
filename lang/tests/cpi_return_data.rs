...
fn test_cpi_return_data_snapshot_keeps_original_bytes() {
    ...
    let snapshot = CpiReturnData::new(program_id, original_bytes.clone());

    let spoofed = 999u64;
    let spoofed_bytes = borsh::to_vec(&spoofed).unwrap();
    let later = CpiReturnData::new(program_id, spoofed_bytes.clone());

    let (snapshot_program_id, snapshot_bytes) = snapshot.return_data();
    assert_eq!(snapshot_program_id, program_id);
    assert_eq!(snapshot_bytes, original_bytes.as_slice());

    let (later_program_id, later_bytes) = later.return_data();
...
fn test_cpi_return_data_snapshot_rejects_program_id_mismatch() {
    ...
    let value = 10u64;
    let snapshot = CpiReturnData::new(other_program_id, borsh::to_vec(&value).unwrap());
