...
/// A data structure of accounts that can be deserialized from the input to a
/// Solana program. This trait handles the **deserialization** phase of the
/// account lifecycle: reading accounts from the transaction, allocating
/// initialised accounts (`init`, `zeroed`, `realloc`), and populating PDA bump
/// seeds into `ctx.bumps`.
///
/// Constraint checking (the **validation** phase) has moved to the
/// [`Validate`] trait, which runs after `try_accounts` and before the user
/// handler. In most cases, it's recommended to use the
/// [`Accounts`](./derive.Accounts.html) derive macro (with `#[validate]`) to
/// implement both traits automatically.
///
/// Generics:
...
pub trait Accounts<'info, B>: ToAccountMetas + ToAccountInfos<'info> + Sized {
    /// Deserializes accounts from the transaction input and runs the
    /// **initialization** phase of the account lifecycle: allocating `init` /
    /// `zeroed` / `realloc` accounts, and populating PDA bump seeds.
    ///
    /// Access-guard constraint checks (owner, signer, `has_one`, `address`,
    /// token/mint guards, etc.) have moved to [`Validate::validate`] which
    /// runs immediately after this method.
    ///
