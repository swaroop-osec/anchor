...
Copies the generated `<program_name>.json{:file}` IDL to the given directory on `$ <blue>anchor</blue> build`. Use this when another tool or a versioned frontend needs the JSON outside the workspace `target{:dir}` directory:
